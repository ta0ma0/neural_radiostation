...
# Сторонние модули
from ai_connector_eng import generate_dj_speech
from last_fm import main as search_artist_info
from voice_engine_eng import AlyxVoice
...
LOG_FILE = "django-aws-terminal-websocket/dj_alyx_radio.log"
...
# Теперь замените принты в коде, например:
# tty_log(f"Подъем Master-узла вещания...", "info")
# tty_log(f"{os.path.basename(abs_path)}", "on_air")
...
class CyberRadio:
    ...
    async def start_master_stream(self):
        ...
        tty_log("[*] [System]: Starting Master broadcast node...")
        ...
        if self.master_stream.returncode is not None:
            ...
            tty_log(f"[❌] FFmpeg failed to start! Error: {err.decode()}")
        else:
            tty_log("[✅] Master node is broadcasting at http://localhost:8000/djalyx")
    ...
    async def play_single_file(self, file_path):
        ...
        try:
            ...
            tty_log(f"[!] Error during audio transmission: {e}")
    ...
    async def background_speech_generator(self, track):
        ...
        try:
            ...
            if speech_files:
                ...
                tty_log(f"[⚙️ AI] Broadcast block for {artist_name} is ready.")
    ...
    async def run_radio(self):
        ...
        while self.is_running:
            ...
            if (
                ...
                tty_log(f"[DJ BLOCK] Block prepared: {data['track']['artist']}")
            ...
            if self.playlist:
                ...
                if is_music and not is_special:
                    ...
                    tty_log(
                        f"[*] Block progress: {self.tracks_played_counter}/{self.min_tracks_before_dj}"
                    )
